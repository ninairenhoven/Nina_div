...
class HTMLPDFGenerator:
    ...
    def process_chunk(self, chunk_df, output_folder):
        """
        ...
        with ProcessPoolExecutor(
            ...
            with tqdm(total=len(futures), desc=f"Progress: ") as pbar:
            ...
            self.logger.info(f"Chunk complete: {chunk_success} success, {chunk_failed} failed")
            gc.collect()
    ...
    def generate(self):
        ...
        try:
            ...
            for chunk_num in range(num_chunks):
                ...
                self.logger.info(f"\n\nProcessing chunk {chunk_num+1} (Generate {nrows} files)...")
                self.process_chunk(chunk_df, self.output_folder)

                # Estimate remaining time
                chunk_duration = (datetime.now() - chunk_start_time).total_seconds()
                files_remaining = files_remaining - nrows
                
                est_remaining_seconds = (chunk_duration / nrows) * files_remaining
                est_finish = datetime.now() + timedelta(seconds=est_remaining_seconds)
                
                self.logger.info(f"⏱️ {files_remaining} files remaining. Estimated {est_remaining_seconds/60:.1f} min, done at ~{est_finish.strftime('%H:%M:%S')}")
            ...
            print("\n\n⚠️  Avbrutt av bruker (Ctrl-C)")
            print("⏳ Avslutter workers... (dette kan ta noen sekunder)")
...
def select_batches(df):
    ...
    if len(use_df)>10000:
        check_input = input('❗ NB: More Than 10 000 rows. [Y] to continue: ').lower()
        if check_input != 'y':
            sys.exit()
...
def generate_pdfs(template, lookup_df):
    ...
    logger.info(f"HTML-templatet er lagret til: {html_output_path}\n")
